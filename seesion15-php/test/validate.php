...
<body>
	<div id="container">
		<?php 
			...
			if(isset($_POST['register'])){
				...
				if(empty($_POST['name'])||empty($_POST['username'])||empty($_POST['password'])
						||empty($_POST['gender'])||empty($_POST['city'])){
					$err= "xin vui lòng điền đầy đủ thông tin";
					if ($name=='') {
						$errorName ="xin vui lòng điền name";
					}
					if ($username=='') {
						$errorUserName ="xin vui lòng điền username";
					}
					if ($password=='') {
						$errorPassword ="xin vui lòng điền password";
					}
					if ($gender=='') {
						$errorGender ="xin vui lòng điền gender";
					}
					if ($city=='') {
						$errorCity="xin vui lòng điền city";
					}
				}
				else{
		...
		<form action="#" method="POST" id="form">
			...
			<p> Name :
				 <input type="text" name="name"  id= "btn1" class="btn" value="<?php echo $name; ?>" >
